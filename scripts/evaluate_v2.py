...
使用评测数据集测试 Agent V2 (ToolCoordinator) 的分类准确性
"""
import sys
import os
import json
...
def run_evaluation(dataset_path: str, limit: int = None):
    """运行评测"""
    print("=" * 80)
    print("故障分析 Agent V2 (ToolCoordinator) 评测")
    print("=" * 80)
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump({
            "agent_version": "V2 (ToolCoordinator)",
            "summary": {
                "total": total,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="故障分析 Agent V2 评测")
