...
P1（高优先级）- 核心服务明显故障或性能严重下降：
- 核心服务（推荐、搜索、广告）延迟严重或超时率高（> 10%）
- 数据库死锁、慢查询严重影响性能
- 服务频繁重启、OOM、内存/磁盘即将耗尽
- 消息队列严重堆积
- 部署后出现明显问题
...
类别判断：
- availability: 可用性问题（错误、故障、不可用、服务异常）
- latency: 延迟问题（慢、超时、响应时间长）
- database: 数据库问题（死锁、慢查询、连接池）
- deployment: 部署问题（部署后出现、回滚相关）
- unknown: 不确定
...
重要：基于初步判断和证据综合判断，如果证据支持初步判断的 severity，应保持一致。"""
