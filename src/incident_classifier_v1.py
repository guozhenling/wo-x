...
P0（最高优先级）- 核心收入或数据安全受影响（仅限以下）：
- 支付、订单创建失败或成功率明显下降（影响核心收入）
- 核心服务完全不可用或错误率 > 10%（注意：必须是核心收入服务 + 错误率超 10%）
- 用户完全无法登录或注册
- 数据丢失或安全漏洞

重要提示：延迟问题、超时问题通常不是 P0，除非导致核心收入功能完全不可用

P1（高优先级）- 核心服务明显故障（务必准确判断）：
- 核心服务延迟严重或超时率 > 10%（包括广告服务）
- 错误率 5-10%（虽未达 P0，但已严重影响用户体验）
- 数据库死锁、连接池耗尽、慢查询严重
- API Gateway、核心服务频繁 OOM、每 10 分钟重启（服务极不稳定）
- 部署后错误日志暴增、错误率上升

关键：OOM 导致频繁重启（如每 10 分钟一次）= P1，因为服务随时可能完全挂掉
...
严重程度标准（务必严格遵守）：
- P0: 核心收入功能故障（仅限支付/订单/登录完全不可用，或核心收入服务错误率 > 10%）
  * 延迟/超时问题通常不是 P0，除非导致完全不可用
  * 广告服务不是 P0（不直接影响核心收入）

- P1: 核心服务明显故障（重点关注）
  * 错误率 5-10%（虽未达 P0，但已严重影响用户）
  * 延迟严重或超时率 > 10%（包括广告、推荐等核心服务）
  * 数据库死锁、连接池耗尽、慢查询严重
  * API Gateway 或核心服务频繁 OOM、每 10 分钟重启（极不稳定，随时可能完全挂掉）
  * 部署后错误日志暴增、错误率上升

- P2: 非核心功能故障，错误率 1-5%，已回滚恢复
- P3: 低错误率（< 1%），偶发问题，无需立即处理

关键判断点：
1. 延迟/超时问题 → 通常是 P1（不是 P0，除非完全不可用）
2. 广告服务问题 → 最高 P1（不是 P0，因为不直接影响核心收入）
3. OOM 频繁重启（每 10 分钟）→ P1（服务极不稳定）
4. 错误率 8% → P1（不是 P2）
